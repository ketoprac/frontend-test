...
const Disclosure = (props: any) => {
  ...
  return (
    <>
      ...
      >
        <button>{title}</button>
        <img className={`h-4 ${expand && "rotate-180"}`} src={ChevronDown} alt="" />
      </div>
      {
      ...
      isLoading ? (<p>Loading...</p>) :
          repos && repos.length > 1 ? repos.map((repo: any) => (
          <Card repoTitle={repo.name} repoDesc={repo.description} repoStars={repo.stargazers_count} repoUrl={repo.html_url} />
